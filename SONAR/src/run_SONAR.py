...
from get_PPI_feature_mat_NewRBPlist_v4 import get_PPI_features
from generate_RCS_score_table import get_scores_for_allprot_via_cvTesting_oversampled
...
parser.add_argument('-e', '--Edge_list', dest='Edgelist', help='The file that store the PPI network in the format of edge-list. Please refer to the format of the edge-list file in folder Demo', metavar='FILE')
parser.add_argument('-r', '--RBP_list', dest='RBPlist', help='A text file that store', metavar='FILE')
parser.add_argument('-p', '--processes', dest='processes', type='int', default=4, help='The number of processes you wanna start for this program', metavar='num_processes')
parser.add_argument('--outfile_feature_table', dest='PPI_feature_table', help='', default='./PPI_feature_table.xls', metavar='FILE')
parser.add_argument('-o', '--outfile', dest='outfile', default='./SONAR_score_table.txt', help='The output of the program which contains the SONAR scores for each proteins', metavar='FILE')
parser.add_argument('-R', '--Repeats', dest='repeats', help='The times to repeat when caluculating the SONAR scores.', type='int', default=10, metavar='INT') 
